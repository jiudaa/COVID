...
from policy_AE import policy_AE
...
country = data[data['CountryName']=='United Arab Emirates']
...
datelist = [[20200301,20200401,0.9],[20200308,20200408,0.9],[20200325,20200425,0.9],[20210103,20210203,0.99],[20210126,20210226,0.9],[20210316,20210416,0.9]]
for i in range(len(datelist)):
    ...
    m.add_country_holidays(country_name='AE')
    m.fit(df_renamed)
    ...
    plt.vlines(datetime.strptime(str_policy_date, '%Y-%m-%d'),0,3000,colors='r')
    plt.savefig('UnitedArabEmirates_' + str(policy_date), dpi=600)
    # plt.show()
